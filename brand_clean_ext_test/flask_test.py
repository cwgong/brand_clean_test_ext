...
class BrandClean(Resource):
    def __init__(self):
        self.cate_name = "shaokao"
        self.bc_date = "2019-12-01"
        ...
        if self.cate_name not in bc_config.cateName2Dir_Dict or \
            ...
            raise Exception('unkown cate_name: %s' % self.cate_name)
        # dir-path checking
        inc_data_folder = bc_config.cateName2Dir_Dict[self.cate_name]
        if not os.path.exists(inc_data_folder):
            raise Exception('%s does not exist!' % inc_data_folder)
        ...
        log_instance.error("cate-name: %s bc_date: %s" % (self.cate_name, self.bc_date))
        pass
    ...
    def _inc_brand_clean(self):
        ...
        try:
            ...
            incObj = IncBrandReg(bc_config.cateName2Dir_Dict[self.cate_name], log_instance)
