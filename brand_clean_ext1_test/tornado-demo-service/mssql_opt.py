...
class MsSqlOpt(object):
    def __init__(self, cate_name, bc_date, log_instance):
        ...
        inc_data_result = bc_config.cateName1Dir_Dict[cate_name] + "/inc_data/dp_brands_result.txt.inc." + self.bc_date
    ...
    def getting_legal_brand(self,cateName):
        try:
            legal_brand_dict = {}
    ...
    def add_inc_brand_data(self,cate_name):
        try:
            ...
            try:
                ...
                legal_brand_dict = {}
                for cateName in cateName_list:
                    legal_brand_dict_ = self.getting_legal_brand(cateName)
            ...
            with open(self.inc_data_result) as f1:
                for line in f1:
                    ...
                    if brand not in legal_brand_dict:               #此处是对取到的所有数据进行全量数据清洗，将全量数据中能匹配到合法品牌的数据存储到数据库当中，否则就略过
                    ...
                    #从之前的result文件中获取到结果，经过处理后得到相对应的各个元素，存入数据库
