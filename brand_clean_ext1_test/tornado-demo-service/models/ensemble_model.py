...
class Ensemble_Model(object):
    ...
    def cate_clean_v2(self):
        ...
        try:
            ...
            old version by yangjiangtao
    '''
    def cate_clean(self,cate_name,bc_date,mssql_opt):
        # checking
        if cate_name not in bc_config.cateName1Dir_Dict:
            return {"status": -1, "info": 'unkown cate_name: %s' % cate_name}
        # dir-path checking
        inc_data_folder = bc_config.cateName1Dir_Dict[cate_name]
        if not os.path.exists(inc_data_folder):
            return {"status": -1, "info": '%s does not exist!' % inc_data_folder}
        try:
        ...
        try:
            ...
            #此处传递的是一级类目，存的过程中存储三次
            r_no = mssql_opt.add_inc_brand_data(cate_name)
    ...
    def _getting_data_from_hive(self,cate_name,bc_date):
        ...
        lst1 = []
        inc_data_file = bc_config.cateName1Dir_Dict[cate_name] + "/inc_data/inc_data.txt"
        ...
        try:
            ...
            for cateName in cateName_list:
                self._getting_data_from_hive_single(cur,cateName,lst1)
                '''
                lst1 = []
                results = cur.fetchall()
                log_instance.info("hive-3")
    ...
    def _inc_brand_clean(self,cate_name,bc_date):
        ...
        try:
            incObj = IncBrandReg(bc_config.cateName1Dir_Dict[cate_name], log_instance, bc_date)
    ...
    def _liwei_brand_clean(self,cate_name,bc_date,mssql_opt):
        try:
            #此处的cateName是二级分类的类别，而cate_name是一级分类的类别，传入二级分类找到legal_brand_dict,传入一级分类找到一级分类下的文件目录
            # legal_brand_dict = mssql_opt.getting_legal_brand(cate_name)
            cateName_list = bc_config.one2two_cateName[cate_name]
            legal_brand_dict = {}
            for cateName in cateName_list:
                legal_brand_dict_ = mssql_opt.getting_legal_brand(cateName)
                legal_brand_dict = dict(legal_brand_dict, **legal_brand_dict_)
            liwei_obj = LiweiBrandClean(legal_brand_dict,
                                        bc_config.cateName1Dir_Dict[cate_name],
